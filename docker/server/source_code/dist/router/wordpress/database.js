...
async function build(req, res) {
  try {
    ...
    let exportdb = await query.backupDatabase(config["DB_USER"], config["DB_PASSWORD"], config["DB_NAME"]);
    ...
    let importdb = await query.importDatabase(config["DB_USER"], config["DB_PASSWORD"], config["DB_NAME"], config["DB_HOST"], file[file.length - 1].slice(11));

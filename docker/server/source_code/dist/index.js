...
function replace(domain, framework) {
	switch (framework) {
		...
		case 'wordpress':
			creatdb(domain).then(function (connect) {
				var datas = connect.database;
				connect.connect.destroy(function (er) {
					if (er) throw er;
				});
				...
				exec(imp, function (err) {
					...
					connection.getConnection(function (errr, connectt) {
						...
						connectt.query('SELECT option_value FROM wp_options WHERE option_name = \'siteurl\'', function (error, results) {
							if (error) throw error;
							if (results) exec('php srdb.cli.php -u root -pxuankhu123s -n ' + datas + ' -s ' + results[0].option_value + ' -r ' + domain + ' -h localhost', function (err2) {
								if (err2) throw err2;
							});
							connectt.destroy();
						});

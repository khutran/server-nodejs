...
export class Query {
  info() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const use_ram = await spawn("awk", ['{print $1,"|total:"$2,"|used:"$3,"|free:"$4,"|shared:"$5,"|cache:"$6,"|available:"$7"-"}'], {
          capture: ["stdout"]
        }).progress(childProcess => {
          childProcess.stdin.write(grep.stdout);
          childProcess.stdin.end();
        });
        let r = _.split(use_ram.stdout, "-").filter(item => {
          return item !== "\n";
        }).map(item => {
          item = _.split(item, "|");
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (process.env.PATH_WEB) {
          cmd = this.convertCommand(`chown -R ${user}:${group} ${process.env.PATH_WEB}/${website}`);
        }
  ...
  exportDatabase(user, password, host, dbname) {
    return new Promise(async (resolve, reject) => {
      try {
        if (Boolean(process.env.MYSQL_ON) === true) {
          let ex = await exec(`mysqldump -u ${user} -p${password} ${dbname} -h ${host} > database/${dbname}.sql`);
  ...
  backupDatabase(user, password, host, dbname) {
    return new Promise(async (resolve, reject) => {
      try {
        if (Boolean(process.env.MYSQL_ON) === true) {
          let ex = await exec(`mysqldump -u ${user} -p${password} ${dbname} -h ${host} > ${process.env.PATH_WEB}/${dbname}.sql`);
          resolve({ success: true, database: `${dbname}.sql` });
        } else {
          reject({
            message: "framework can not database",
  ...
  importDatabase(user, password, dbname, host, impotdb) {
    return new Promise(async (resolve, reject) => {
      try {
        if (Boolean(process.env.MYSQL_ON) === true) {
          await exec(`mysql -u ${user} -p${password} ${dbname} -h ${host} < database/${impotdb}`);
          resolve({ success: true });
        } else {
          reject({
            message: "framework can not database",

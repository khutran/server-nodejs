...
export default class WordpressQuery extends Query {
  ...
  createWpConfig(website) {
    return new Promise(async (resolve, reject) => {
      try {
        if (fs.existsSync(`${process.env.PATH_WEB}/${website}/workspace/wp-config.php`) === true) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (Boolean(process.env.MYSQL_ON) === true) {
          ...
          let db = await this.exportDatabase(config["DB_USER"], config["DB_PASSWORD"], config["DB_HOST"], config["DB_NAME"]);
          ...
          await this.importDatabase(data["DB_USER"], data["DB_PASSWORD"], data["DB_NAME"], db["database"]);
          let webold = await models.sequelize.query(`SELECT \`option_value\` FROM \`${data["DB_NAME"]}\`.\`${config["PREFIX"]}options\` WHERE \`option_name\` = 'siteurl'`, { type: models.sequelize.QueryTypes.SELECT });
          webold = webold[0].option_value;

          let replace = await this.replaceUrl(data["DB_NAME"], config["PREFIX"], webold, `${firts}${webnew}`);
  ...
  dump(res) {
    return new Promise(async (resolve, reject) => {
      if (Boolean(process.env.MYSQL_ON) === true) {
        ...
        let config = await this.readConfig("wp-config.php");
        var sp = spawncmd("mysqldump", ["-u" + config["DB_USER"], "-p" + config["DB_PASSWORD"], "-h" + config["DB_HOST"], config["DB_NAME"], "--default-character-set=utf8", "--comments"], {
          highWaterMark: 16 * 1024
        });
        res.setHeader("Content-Type", "application/octet-stream");
        res.setHeader("Content-disposition", `filename=${config["DB_NAME"]}.sql`);
        sp.stdout.pipe(res).on("finish", function () {
          resolve();
    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (Boolean(process.env.MYSQL_ON) === true) {
          ...
          await this.backupDatabase(config["DB_USER"], config["DB_PASSWORD"], config["DB_HOST"], config["DB_NAME"]);
          ...
          await this.importDatabase(config["DB_USER"], config["DB_PASSWORD"], config["DB_NAME"], config["DB_HOST"], file[file.length - 1].slice(11));

          let webold = await models.sequelize.query(`SELECT \`option_value\` FROM \`${config["DB_NAME"]}\`.\`${config["PREFIX"]}options\` WHERE \`option_name\` = 'siteurl'`, { type: models.sequelize.QueryTypes.SELECT });
          webold = webold[0].option_value;

          await this.replaceUrl(config["DB_NAME"], config["PREFIX"], webold, website);

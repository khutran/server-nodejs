  ...
  clone(domain, git, branch, key, secret) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let cmd1 = this.convertCommand(`git fetch --tags --progress ${urlGit} +refs/heads/*:refs/remotes/origin/*`);
        let cmd2 = this.convertCommand(`git config remote.origin.url ${urlGit}`);
        let cmd3 = this.convertCommand(`git config --add remote.origin.fetch +refs/heads/*:refs/remotes/origin/*`);
        let cmd4 = this.convertCommand(`git config remote.origin.url ${urlGit}`);
        let cmd5 = this.convertCommand(`git fetch --tags --progress ${urlGit} +refs/heads/*:refs/remotes/origin/*`);
        let cmd6 = this.convertCommand(`git rev-parse refs/remotes/origin/${branch}^{commit}`);
        let cmd7 = this.convertCommand(`git rev-parse refs/remotes/origin/origin/${branch}^{commit}`);
  ...
  pull(domain, git, branch, key, secret) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let cmd2 = this.convertCommand(`git config remote.origin.url ${urlGit}`);
        let cmd3 = this.convertCommand(`git fetch --tags --progress ${urlGit} +refs/heads/*:refs/remotes/origin/*`);
        let cmd4 = this.convertCommand(`git rev-parse refs/remotes/origin/${branch}^{commit}`);
